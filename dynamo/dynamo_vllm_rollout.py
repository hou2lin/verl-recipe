...
"""ServerAdapter for the dynamo backend on the vLLM engine.

Inherits the vLLM ServerAdapter (the HTTP path is identical: trainer rank reads
``replica.server_address`` and POSTs chat completions to it) and only overrides
the Ray actor name used for sleep/wake/``update_weights`` RPC, so it lands on
``dynamo_server_*`` (created by ``DynamoReplica.launch_servers``) rather than
``vllm_server_*``.

Imported lazily by ``dynamo_rollout.ServerAdapter`` and never at module scope:
verl's ``workers/rollout/vllm_rollout/__init__`` raises on a vLLM-free image, so
