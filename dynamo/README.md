...
| `free_engine_on_train` | mirrors `rollout.free_cache_engine` | Free the engine (sleep) during the training phase. No longer an independent switch: it follows `rollout.free_cache_engine` (default `true`) and an explicit value that contradicts it fails at startup. The sglang engine additionally requires `rollout.enable_sleep_mode=true` when this is on (that flag adds `--enable-memory-saver`) and refuses to start otherwise. This is what makes CUDA graph affordable on tight-memory footprints. |
...
- Dynamo, sglang engine: additionally requires the incremental-logprobs fix
  ...
  [sibling-repo patches](#patches-for-repos-outside-this-recipe) applied, or
