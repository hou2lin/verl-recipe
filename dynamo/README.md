...
re-hit instead of being recomputed. The CPU tier survives rollout rounds and
weight syncs (the connector is reset as part of the pause/resume choreography).
Combined with the KV-aware router this completes the KV-reuse stack for
...
| Component | Version |
|---|---|
| verl | `main` (tested at the commit in `REQUIRED_VERL.txt`) |
...
`VERL_DYNAMO_FE_READY_TIMEOUT` (default 2400 s; first-load plus compile of a
large model can exceed the old 600 s window). Multi-shard nodes are isolated
automatically (per-shard IPC socket, instance ids, metric-port offsets);
`FLEXKV_SHARED_CPU_CACHE=1` opts into one shared CPU pool instead.

**Sizing rule (important):** keep the active working set inside the GPU cache,
`C_per_engine × mean_context_tokens ≲ 0.7 × gate_tokens`, where `gate_tokens =
kv-cache-memory-bytes / kv_bytes_per_token`. Operating far above this puts the
current FlexKV implementation into an allocator-starvation regime (transfer
pins defeat vLLM preemption) that can stall the engine; fixes are being
reported upstream.
...
```bash
# backend smoke, 1+1 GPU (trainer / standalone rollout), 2 RL steps
MODEL_PATH=<untied model> TRAIN_FILE=... TEST_FILE=... \
  bash dynamo/smoke_dynamo_fully_async.sh \
  actor_rollout_ref.actor.fsdp_config.offload_policy=True

# FlexKV L2 acceptance: same layout, FlexKV on
FLEXKV=1 MODEL_PATH=... TRAIN_FILE=... TEST_FILE=... \
  bash dynamo/smoke_dynamo_fully_async.sh \
  actor_rollout_ref.actor.fsdp_config.offload_policy=True
```

Both print `PASS: Dynamo fully_async smoke completed` and leave the verl tree
`git status`-clean.
