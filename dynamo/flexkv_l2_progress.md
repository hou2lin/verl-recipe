...
| Gate B — long-trajectory probe | ✅ **CLOSED 2026-08-17 — no pressure at C32; three arms identical** | 131k ctx / 100 turns / 64 traj / C32 / 0.68: bare-vs-no-pin-vs-pin all at **GPU hit 97.7%**, recompute 2.3%, FlexKV hit ≈0, needs-L2 = 1.4% of requests, catch 0%. Pin mechanism perfect (2,719/2,719 pinned, 0 rejected) but jobless. Cause: trajectories naturally end at ~86k avg (21k avg prompt), C32 active set ≈1.38M tok < 1.55M pool → **zero eviction**. Verdict: "long trajectory" alone is NOT the L2 regime — **oversubscription is**; length only raises the per-eviction restore cost (c²). Wall: 64/68/81 min (pin arm +13 min unexplained, single sample). Two ops notes: launcher OOM false start (batched-tokens coupling, fixed); 131k-domain join-rate summary bug (exit 8 both arms, rollouts complete — metrics computed from traces directly) |
| Gate B′ — long-trajectory **oversubscribed** probe (proposed) | ⏳ awaiting go | same config but **C64** (single wave of 64): active set ≈2.75M vs 1.55M pool ≈ 1.8× oversubscription → eviction + c² restore on the critical path. Arms ±pin, est. 1.5–2.5h each |
