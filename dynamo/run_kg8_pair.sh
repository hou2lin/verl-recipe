...
GATE_VAL=${GATE_VAL:-20401094656}   # 19 GiB/rank
ST=/workspace/phase2/kg8pair.status
EV=/workspace/phase2/kvrouter_evidence
mkdir -p $EV
echo "[kg8pair] $(date -u +%FT%TZ) START jsonl=$JSONL gate=$GATE_VAL" >> $ST
for ARM in 1 0; do
  echo "[kg8pair] $(date -u +%FT%TZ) serving arm$ARM" >> $ST
  ...
  if ! curl -sf http://localhost:8500/v1/models | grep -q qwen3-32b; then
    echo "[kg8pair] $(date -u +%FT%TZ) ARM$ARM SERVE_FAILED" >> $ST
    exit 1
  fi
  echo "[kg8pair] $(date -u +%FT%TZ) arm$ARM ready, paced replay" >> $ST
  python3 /workspace/dynres/pkgroot/recipe/dynamo/frozen_replay.py \
    --endpoint http://localhost:8500 --model qwen3-32b \
    --trajectories $JSONL --concurrency 16 --warm-rounds 1 --steady-rounds 4 \
    --paced --think-time 2.0 --converge-pct 5 \
    --out /workspace/phase2/kg8_arm${ARM}_19g.json \
  ...
  echo "[kg8pair] $(date -u +%FT%TZ) arm$ARM replay rc=$rc" >> $ST
done
# teardown
...
echo "[kg8pair] $(date -u +%FT%TZ) EXIT_OK" >> $ST
